...
#from PySide6 import PYQT5_API
#from PySide6 import PYQT4_API
#from PySide6 import PYSIDE_API
...
class PromptLineEdit(QtWidgets.QLineEdit):
    ...
    def set_button_visible(self, visible):
        ...
        left = self.textMargins().left()  # .getTextMargins()
        top = self.textMargins().top()
        right = self.textMargins().right()
        bottom = self.textMargins().bottom()
        if visible:
