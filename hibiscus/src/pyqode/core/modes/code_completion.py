...
class CodeCompletionMode(Mode, QtCore.QObject):
        ...
        if event.text():
            ...
            if event.text() in self._trigger_symbols:
                ...
                if event.modifiers() in [
                        QtCore.Qt.KeyboardModifier.NoModifier, QtCore.Qt.KeyboardModifier.ShiftModifier]:
                    self.request_completion()
                else:
                    self._hide_popup()
            else:
    ...
    def _is_shortcut(self, event):
        ...
        valid_modifier = (event.modifiers() & modifier) == modifier
