...
class TextDecoration(QtWidgets.QTextEdit.ExtraSelection):
    ...
    def __init__(self, cursor_or_bloc_or_doc, start_pos=None, end_pos=None,
        ...
        if end_pos is not None:
            self.cursor.setPosition(end_pos, QtGui.QTextCursor.MoveMode.KeepAnchor)
        if start_line is not None:
            self.cursor.movePosition(QtGui.QTextCursor.MoveOperation.Start, 
                                    QtGui.QTextCursor.MoveMode.MoveAnchor)
            self.cursor.movePosition(QtGui.QTextCursor.MoveOperation.Down, 
                                    QtGui.QTextCursor.MoveMode.MoveAnchor,
                                    start_line)
    ...
    def select_line(self):
        ...
        :return:
        """
        self.cursor.movePosition(QtGui.QTextCursor.MoveOperation.StartOfBlock)
        text = self.cursor.block().text()
        lindent = len(text) - len(text.lstrip())
        self.cursor.setPosition(self.cursor.block().position() + lindent)
        self.cursor.movePosition(QtGui.QTextCursor.MoveOperation.EndOfBlock,
                                 QtGui.QTextCursor.MoveMode.KeepAnchor)
