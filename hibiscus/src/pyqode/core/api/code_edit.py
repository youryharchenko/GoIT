...
class CodeEdit(QtWidgets.QPlainTextEdit):
    ...
    **Managers, Modes and Panels**.
    ...
    def insert_action(self, action, prev_action):
        ...
        """
        if isinstance(prev_action, QtGui.QAction):
    ...
    def _init_actions(self, create_standard_actions):
        ...
        if create_standard_actions:
            # Undo
            action = QtGui.QAction(_('Undo'), self)
            ...
            # Redo
            action = QtGui.QAction(_('Redo'), self)
            ...
            # Copy
            action = QtGui.QAction(_('Copy'), self)
            ...
            # cut
            action = QtGui.QAction(_('Cut'), self)
            ...
            # paste
            action = QtGui.QAction(_('Paste'), self)
        ...
        # duplicate line
        action = QtGui.QAction(_('Duplicate line'), self)
        action.setShortcut('Ctrl+D')
        action.triggered.connect(self.duplicate_line)
        self.add_action(action, sub_menu=None)
        self.action_duplicate_line = action
        # swap line up
        action = QtGui.QAction(_('Swap line up'), self)
        ...
        # swap line down
        action = QtGui.QAction(_('Swap line down'), self)
        action.setShortcut(QtGui.QKeySequence(
        ...
        # select all
        action = QtGui.QAction(_('Select all'), self)
        ...
        if create_standard_actions:
            # indent
            action = QtGui.QAction(_('Indent'), self)
            ...
            # unindent
            action = QtGui.QAction(_('Un-indent'), self)
            ...
            self.add_separator()
        # goto
        action = QtGui.QAction(_('Go to line'), self)
    ...
    def _init_settings(self):
        ...
        self._tab_length = 4
        self._use_spaces_instead_of_tabs = True
        #self.setTabStopWidth(self._tab_length * self.fontMetrics().horizontalAdvance(" "))
        self.setTabStopDistance(self._tab_length * self.fontMetrics().horizontalAdvance(" "))
