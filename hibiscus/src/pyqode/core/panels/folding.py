...
from PySide6 import QtCore, QtWidgets, QtGui #, PYQT5_API
...
class FoldingPanel(Panel):
    ...
    def on_install(self, editor):
        ...
        action = self.action_collapse = QtGui.QAction(
            _('Collapse'), self.context_menu)
        action.setShortcut('Shift+-')
        action.triggered.connect(self._on_action_toggle)
        self.context_menu.addAction(action)
        action = self.action_expand = QtGui.QAction(_('Expand'),
                                                        self.context_menu)
        action.setShortcut('Shift++')
        action.triggered.connect(self._on_action_toggle)
        self.context_menu.addAction(action)
        self.context_menu.addSeparator()
        action = self.action_collapse_all = QtGui.QAction(
        ...
        self.context_menu.addAction(action)
        action = self.action_expand_all = QtGui.QAction(
