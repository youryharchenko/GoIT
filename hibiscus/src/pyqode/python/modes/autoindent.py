...
class PyAutoIndentMode(AutoIndentMode):
    ...
    def _get_last_word(tc):
        tc2 = QTextCursor(tc)
        tc2.movePosition(QTextCursor.MoveOperation.Left, QTextCursor.MoveMode.KeepAnchor, 1)
        tc2.movePosition(QTextCursor.MoveOperation.WordLeft, QTextCursor.MoveMode.KeepAnchor)
    ...
    def _get_indent_of_opening_paren(self, tc):
        ...
        tc.movePosition(QTextCursor.MoveOperation.Right, QTextCursor.MoveMode.MoveAnchor)
        mapping = {
            ')': (OPEN, PAREN),
            ']': (OPEN, SQUARE),
            '}': (OPEN, BRACE)
    ...
    def _get_first_open_paren(self, tc, column):
        ...
        while ln >= 0 and tc.block().text().strip():
            tc_trav.movePosition(QTextCursor.MoveOperation.StartOfLine, QTextCursor.MoveMode.MoveAnchor)
                ...
                if paren.position < column:
                    if self._is_paren_open(paren):
                        if paren.position > column:
                            ...
                            return pos, char
            # check previous line
            tc_trav.movePosition(QTextCursor.MoveOperation.Up, QTextCursor.MoveMode.MoveAnchor)
    ...
    def _get_prev_char(tc):
        tc2 = QTextCursor(tc)
        tc2.movePosition(QTextCursor.MoveOperation.PreviousCharacter, QTextCursor.MoveMode.KeepAnchor)
        char = tc2.selectedText()
        while char == ' ':
            tc2.movePosition(QTextCursor.MoveOperation.PreviousCharacter, QTextCursor.MoveMode.KeepAnchor)
            char = tc2.selectedText()
    ...
    def _handle_indent_between_paren(self, column, line, parent_impl, tc):
        ...
        if is_string:
            ...
            while self._helper.is_comment_or_string(
                    trav, formats=['string']):
                trav.movePosition(QTextCursor.MoveOperation.Left)
            trav.movePosition(QTextCursor.MoveOperation.Right)
