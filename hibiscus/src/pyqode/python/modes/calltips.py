...
class CalltipsMode(Mode, QtCore.QObject):
    ...
    def _is_last_chard_end_of_word(self):
        try:
            tc: QtGui.QTextCursor = TextHelper(self.editor).word_under_cursor()
            # tc.setPosition(Q.position())
            tc.movePosition(QtGui.QTextCursor.MoveOperation.StartOfLine, QtGui.QTextCursor.MoveMode.KeepAnchor)
            l = tc.selectedText()
            last_char = l[len(l) - 1]
    ...
    def _display_tooltip(self, call, col):
        ...
        char_width = self.editor.fontMetrics().horizontalAdvance('A')
