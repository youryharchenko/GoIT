...
def calltips(request_data):
    ...
    try:
        ...
        return []
    signatures = script.get_signatures(line=line, column=column) #  call_signatures()
...
def defined_names(request_data):
    ...
    #column = request_data['column']
    path = request_data['path']
    toplvl_definitions = jedi.Script(code).get_names()
...
def quick_doc(request_data):
    ...
    script = jedi.Script(code, path=path) #, line, column, path, encoding)
    try:
        definitions = script.infer(line=line, column=column) # script.goto_definitions()
    except jedi.NotFoundError:
        return []
    else:
...
class JediCompletionProvider:
    ...
    def complete(code, line, column, path, encoding, prefix):
        ...
        ret_val = []
        try:
            script = jedi.Script(code, path=path) # line + 1, column, path, encoding)
